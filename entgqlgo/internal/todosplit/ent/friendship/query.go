...
func (_q *FriendshipQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.Ctx.Fields; len(fields) > 0 {
		...
		if _, ok := _q.eagerLoaders["todo"]; ok {
			_spec.Node.AddColumnOnce(FieldTodoID)
		}
		if _, ok := _q.eagerLoaders["category"]; ok {
			_spec.Node.AddColumnOnce(FieldCategoryID)
		}
	}
