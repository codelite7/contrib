...
// QueryTodo queries the todo edge of a Friendship.
func (c *FriendshipClient) QueryTodo(_m *Friendship) *TodoQuery {
	query := (&TodoClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// QueryCategory queries the category edge of a Friendship.
func (c *FriendshipClient) QueryCategory(_m *Friendship) *CategoryQuery {
...
type (
	hooks struct {
		BillProduct, Category, Friendship, Todo []ent.Hook
	}
	inters struct {
		BillProduct, Category, Friendship, Todo []ent.Interceptor
	}
)
