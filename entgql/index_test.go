...
// TestEmitIndexFile_MultiTable_Golden exercises the inter-table spacing
// that the per-table golden tests above can't cover.
func TestEmitIndexFile_MultiTable_Golden(t *testing.T) {
	t.Parallel()
	cfg := IndexConfig{
		Tables: []IndexTable{
			{
				Name: "alphas",
				Indexes: []Index{
					{Name: "idx_order_alphas_name_id", Field: "name"},
				},
			},
			{
				Name: "zebras",
				Indexes: []Index{
					{Name: "idx_order_zebras_created_at_id", Field: "created_at", Where: "deleted_at IS NULL"},
				},
			},
		},
	}
	assertGoldenMatch(t, cfg, "testdata/indexes/multi_table.expected.sql")
}

// TestEmitIndexFile_Empty_OnlyHeader pins the contract that an empty config
// produces just the file header (no per-table comments, no CREATE statements).
func TestEmitIndexFile_Empty_OnlyHeader(t *testing.T) {
	t.Parallel()
	tmpFile := filepath.Join(t.TempDir(), "indexes.sql")
	err := emitIndexFile(tmpFile, IndexConfig{})
	require.NoError(t, err)

	data, err := os.ReadFile(tmpFile)
	require.NoError(t, err)
	require.Equal(t, "-- Generated by entgql.WithIndexOutput. DO NOT EDIT.\n-- Modify the schema and re-run codegen instead.\n", string(data))
}
