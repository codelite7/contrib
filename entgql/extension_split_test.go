...
func TestGenerateCollectionDispatchPkg_WritesFileWithExpectedContent(t *testing.T) {
	...
	// Minimal graph: Target drives the output path and Header drives the
	// generated header comment (matching every other split-mode template).
	...
	content := string(out)
	require.Contains(t, content, "// Code generated by entgql, DO NOT EDIT.")
...
func TestGenerateCollectionDispatchPkg_OutputCompiles(t *testing.T) {
	...
	// Pin gqlgen to the version contrib's root go.mod uses so the dispatch
	// template's `graphql.OperationContext` / `graphql.CollectedField` API
	// references resolve at build time.
	require.NoError(t, os.WriteFile(filepath.Join(tmp, "go.mod"), []byte(`module example.com/test

go 1.24

require github.com/99designs/gqlgen v0.17.68
`), 0644))
